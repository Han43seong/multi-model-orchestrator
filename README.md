...
단일 모델 한계를 보완하기 위해 슬래시 커맨드 하나로 여러 AI를 배정하고, Evaluator + 상태 머신이 구현 → 검증 → 롤백 사이클을 관리한다. "어떤 모델에게 무엇을 맡길지"를 사람이 반복해서 판단하는 비용을 줄이는 것이 목표다.
...
- 자동 전환·저장·추론 경로는 필요한 CLI, PostgreSQL/pgvector, 설정 파일이 구성된 환경에서 동작한다.
- Evaluator와 상태 머신은 작업 완료를 보장하는 자율 시스템이 아니라, 정해진 계약과 검사 결과에 따라 다음 경로를 선택하는 개발 워크플로다.
- 이 저장소는 실험적 개인 워크플로이며 외부 환경에서는 설치 후 각 연동 경로를 다시 검증해야 한다.
